//In the parallelized version below, the iterations of the loop are split among parallel processes, each iteration is run by only one rank, and each rank has more or less the same amount of work.
